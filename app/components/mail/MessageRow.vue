...
const person = computed(() => {
  const label = who.value ? (who.value.name || who.value.address) : t('mail.row.unknownPerson')
  return props.showRecipient ? t('mail.row.recipientPrefix', { name: label }) : label
})
...
const date = computed(() => formatMessageDate(props.message.date, intlLocale(), dateOpts.value))
const fullDate = computed(() => formatFullDate(props.message.date, intlLocale(), dateOpts.value))
...
<template>
  ...
  >
    ...
    <button
      ...
      :aria-label="message.flagged ? t('mail.row.removeStar') : t('mail.row.addStar')"
    ...
    <div class="flex min-w-0 flex-1 flex-col gap-0.5 @3xl:flex-row @3xl:items-center @3xl:gap-4" :class="compact ? '@3xl:h-9' : '@3xl:h-11'">
      ...
      <div class="flex min-w-0 flex-1 items-start gap-2 @3xl:items-center">
        <NuxtLink
          ...
          @click="onLinkClick"
        >
          <span class="block truncate @3xl:inline" :class="weight">{{ displaySubject(message.subject) }}</span>
        ...
        <div class="flex shrink-0 items-center gap-1.5 text-muted-foreground">
          <Paperclip v-if="message.hasAttachments" class="mt-0.5 size-4 @3xl:mt-0" role="img" :aria-label="t('mail.row.hasAttachment')" />
          <Reply v-if="message.answered" class="size-4" role="img" :aria-label="t('mail.row.answered')" />
          <Forward v-if="message.forwarded" class="size-4" role="img" :aria-label="t('mail.row.forwarded')" />
          <CircleAlert v-if="message.priority === 'high'" class="size-4 text-destructive" role="img" :aria-label="t('mail.row.highPriority')" />
        </div>
        <button
          type="button"
