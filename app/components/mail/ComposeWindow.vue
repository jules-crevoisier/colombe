...
const saveLabel = computed(() => ({
  ...
  saved: t('compose.toasts.draftSaved'),
  error: t('compose.window.saveError'),
}[compose.saveState]))
...
async function onSend() {
  ...
  if (!all.length) {
    toast.error(t('compose.toasts.noRecipient'))
    return
  }
  const invalid = all.filter(a => !validateEmailAddress(a))
  if (invalid.length) {
    toast.error(t('compose.toasts.invalidAddress', { addresses: invalid.join(', ') }))
    return
  }
  if (!compose.subject.trim() && !window.confirm(t('compose.toasts.confirmNoSubject'))) return
...
<template>
  ...
  >
    ...
    >
      ...
      <MailIconButton class="hidden lg:inline-flex" :class="compose.minimized ? 'text-compose-foreground hover:bg-white/10 hover:text-compose-foreground' : ''" :icon="compose.minimized ? ChevronUp : Minus" :label="compose.minimized ? t('compose.window.expand') : t('compose.window.collapse')" @click="compose.minimized = !compose.minimized" />
      <MailIconButton class="hidden lg:inline-flex" :class="compose.minimized ? 'text-compose-foreground hover:bg-white/10 hover:text-compose-foreground' : ''" :icon="compose.expanded ? Minimize2 : Maximize2" :label="compose.expanded ? t('compose.window.exitFullscreen') : t('compose.window.fullscreen')" @click="compose.expanded = !compose.expanded; compose.minimized = false" />
      <MailIconButton :class="compose.minimized ? 'lg:text-compose-foreground lg:hover:bg-white/10 lg:hover:text-compose-foreground' : ''" :icon="X" :label="t('compose.window.saveAndClose')" @click="compose.close()" />
    </header>
    ...
    <form v-show="!compose.minimized" class="relative flex min-h-0 flex-1 flex-col" @submit.prevent="onSend" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
      <div class="px-4 lg:px-5">
        ...
        <div v-if="compose.showIdentityPicker" class="flex min-h-12 items-center gap-1.5 border-b border-border py-1">
          <label for="compose-from" class="w-9 shrink-0 text-sm text-muted-foreground">{{ t('compose.window.from') }}</label>
        ...
        <div class="flex items-start">
          <div class="min-w-0 flex-1">
            <MailRecipientInput id="compose-to" ref="toField" v-model="compose.to" :label="t('compose.window.to')" :autofocus="!compose.to.length" @change="compose.touch()" />
          </div>
          <button v-if="!compose.showCc" type="button" class="mt-0.5 h-11 shrink-0 rounded-md px-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring" @click="compose.showCc = true">{{ t('compose.window.showCcBcc') }}</button>
        </div>
        <template v-if="compose.showCc">
          <MailRecipientInput id="compose-cc" ref="ccField" v-model="compose.cc" :label="t('compose.window.cc')" @change="compose.touch()" />
          <MailRecipientInput id="compose-bcc" ref="bccField" v-model="compose.bcc" :label="t('compose.window.bcc')" @change="compose.touch()" />
        </template>
        <div class="flex min-h-12 items-center border-b border-border">
          <label for="compose-subject" class="sr-only">{{ t('compose.window.subject') }}</label>
          <input id="compose-subject" v-model="compose.subject" :placeholder="t('compose.window.subject')" maxlength="998" class="h-12 w-full bg-transparent font-heading text-lg font-medium outline-none placeholder:font-normal placeholder:text-muted-foreground" @input="compose.touch()">
        </div>
      </div>
      ...
      <div v-else class="min-h-0 flex-1 overflow-y-auto px-4 py-3">
        <Textarea
          ...
          :aria-label="t('compose.editor.label')"
          ...
          :placeholder="t('compose.editor.placeholder')"
          spellcheck="true"
          :lang="currentLocale()"
          @input="compose.touch()"
        />
      ...
      <ul v-if="compose.attachments.length || compose.forwardAsAttachment.length" class="flex flex-col gap-1.5 px-4 pb-2 lg:px-5" :aria-label="t('compose.window.attachmentsListLabel')">
        <li v-for="(name, i) in compose.forwardAsAttachmentNames" :key="`eml-${i}`" class="flex h-11 items-center gap-2 rounded-md border border-border bg-surface-app/60 pr-1 pl-3 text-sm">
          ...
          <button type="button" class="grid size-9 place-items-center rounded-md hover:bg-foreground/10 focus-visible:outline-2 focus-visible:outline-ring" :aria-label="t('compose.window.removeAttachment', { name })" @click="compose.removeForwardedMessage(i)">
        ...
        <li v-for="(a, i) in compose.attachments" :key="`${a.filename}-${i}`" class="flex h-11 items-center gap-2 rounded-md border border-border bg-surface-app/60 pr-1 pl-3 text-sm">
          ...
          <span class="text-xs text-muted-foreground tabular-nums">{{ formatFileSize(a.size, true) }}</span>
          <button type="button" class="grid size-9 place-items-center rounded-md hover:bg-foreground/10 focus-visible:outline-2 focus-visible:outline-ring" :aria-label="t('compose.window.removeAttachment', { name: a.filename })" @click="compose.removeAttachment(i)">
            <X class="size-4" aria-hidden="true" />
          </button>
        </li>
      </ul>
      ...
      >
        <p class="font-heading text-xl font-medium text-primary">{{ t('compose.window.dropFilesHint') }}</p>
      </div>
      ...
      <footer class="flex shrink-0 flex-wrap items-center gap-0.5 border-t border-border px-2 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:gap-1 sm:px-3 lg:px-4">
        ...
        <!-- Send options button -->
        <DropdownMenu>
          <MailMenuButton :icon="Settings" :label="t('compose.window.sendOptions')" />
          <DropdownMenuContent align="start" class="w-56">
            ...
            <DropdownMenuCheckboxItem :model-value="compose.requestReadReceipt" @update:model-value="(v: boolean) => { compose.requestReadReceipt = v; compose.touch() }" @select.prevent>
              {{ t('compose.window.requestReadReceipt') }}
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem :model-value="compose.requestDeliveryReceipt" @update:model-value="(v: boolean) => { compose.requestDeliveryReceipt = v; compose.touch() }" @select.prevent>
              {{ t('compose.window.requestDeliveryReceipt') }}
            </DropdownMenuCheckboxItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <MailIconButton :icon="Trash2" :label="t('compose.window.discardDraft')" @click="compose.discard()" />
      </footer>
    </form>
    ...
    <AlertDialog v-model:open="attachmentReminderOpen">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{{ t('compose.toasts.attachmentReminderTitle') }}</AlertDialogTitle>
        </AlertDialogHeader>
        <AlertDialogDescription>
          {{ t('compose.toasts.attachmentReminderDescription') }}
        </AlertDialogDescription>
        <AlertDialogFooter>
          <AlertDialogCancel>{{ t('compose.toasts.sendAnyway') }}</AlertDialogCancel>
          <AlertDialogAction @click="fileInput?.click()">
            {{ t('compose.toasts.addAttachment') }}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
